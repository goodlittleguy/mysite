...
class LoginForm(forms.Form):
    ...
    def clean(self):
        ...
        if user is None:
            ...
            self.cleaned_data['user'] = user
        return self.cleaned_data  # clean_data是表单内置的存储表单数据(密码,用户名等)的字典参数
...
class RegForm(forms.Form):
    ...
    verification_code = forms.CharField(label="验证码",
                                        required=False,
                                        widget=forms.TextInput(
                                            attrs={'class': 'form-control', 'placeholder': '点击发送验证码到您的邮箱'}))

    password = forms.CharField(label="密码",
                               min_length=6,
                               widget=forms.PasswordInput(
                                   attrs={'class':'form-control','placeholder':'请输入不小于六位的密码'}))
    ...
    def __init__(self, *args, **kwargs):
        if 'request' in kwargs:
            self.request = kwargs.pop('request')  # 下面会检查后继承user是否为本地预定义的,故应该提前将user,pop出来,避免报错
...
class ChangeNicknameForm(forms.Form):
    nickname_new = forms.CharField(label="新的昵称", min_length=3, max_length=10,
                                   widget=forms.TextInput(
                                    attrs={'class': 'form-control', 'placeholder': '请输入新的昵称'}))

    def __init__(self, *args, **kwargs):
        if 'user' in kwargs:
            self.user = kwargs.pop('user')  # 下面会检查后继承user是否为本地预定义的,故应该提前将user,pop出来,避免报错
...
class BindEmailForm(forms.Form):
    email = forms.EmailField(label="邮箱", widget=forms.EmailInput(
                                            attrs={'class': 'form-control', 'placeholder': '请输入您的邮箱号'}))
    verification_code = forms.CharField(label="验证码",
                                        required=False,
                                        widget=forms.TextInput(
                                            attrs={'class': 'form-control', 'placeholder': '点击发送验证码到您的邮箱'}))
    ...
    def clean(self):
        ...
        if self.request.user.email != '':
            raise forms.ValidationError('你已经绑定过该邮箱了')
        return self.cleaned_data
...
class ChangePasswordForm(forms.Form):
    ...
    new_password = forms.CharField(label="请输入您的新密码",
                                   min_length=6,
                                   widget=forms.PasswordInput(
                                       attrs={'class': 'form-control', 'placeholder': '请输入新的密码'}))

    new_password_again = forms.CharField(label="请再次输入您的新密码",
                                         min_length=6,
                                         widget=forms.PasswordInput(
...
class ForgotPasswordForm(forms.Form):
    ...
    new_password = forms.CharField(label="请输入您重置后的密码",
                                   min_length=6,
                                   widget=forms.PasswordInput(
                                       attrs={'class': 'form-control', 'placeholder': '请输入新的密码'}))
    new_password_again = forms.CharField(label="请输入您重置后的密码",
                                         min_length=6,
                                         widget=forms.PasswordInput(
                                            attrs={'class': 'form-control', 'placeholder': '请输入新的密码'}))
